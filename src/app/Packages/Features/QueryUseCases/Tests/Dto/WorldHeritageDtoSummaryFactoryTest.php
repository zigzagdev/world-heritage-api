...
class WorldHeritageDtoSummaryFactoryTest extends TestCase
{
    ...
    public function test_check_return_type_check(): void
    {
        $result = WorldHeritageSummaryFactory::build(self::arrayData());
    ...
    public function test_check_return_data_value(): void
    {
        ...
        $expected = self::arrayData();

        foreach ($expected as $key => $value) {
            $this->assertArrayHasKey($key, $actual, "Missing key {$key} in DTO array");
            $this->assertEquals($value, $actual[$key]);
        }

        $this->assertArrayHasKey('primary_state_party_code', $actual);
        $this->assertSame('SVK', $actual['primary_state_party_code']);
    }
}
